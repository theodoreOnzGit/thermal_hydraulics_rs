...
/// 
/// this first case is a sandbox to parallel arrays of tubes 
/// previously, the dhx shell/tube was modelled as one fluid component 
/// with a particular cross sectional area and hydraulic diameter
/// when in reality there are 19 parallel tubes 
///
/// each pipe has an inner diameter of 0.00635 m
/// and flow area of 6.0172e−4 m2
/// 
/// Now, the hydraulic diameter is 4A/P where P is the wetted perimeter
///
/// increasing the number of tubes will not change the hydraulic diameter 
/// but it will change the total area 
///
/// Re = (mass flow rate * D_H)/(cross sectional area * dynamic viscosity)
///
/// for a constant mass flowrate, constant viscosity and hydraulic diameter,
/// and a lower cross sectional area, 
///
/// Re decreases. and in the laminar regime, we have a higher friction 
/// factor. Modelling a set of parallel tubes as if it were one pipe would 
/// indeed lower the friction factor all else equal.
///
///
/// The tricky part of coding this is in the heat transfer entity portion.
///
/// When heat transfer entities are connected so to speak, there is a 
/// heat transfer interaction that connects two heat transfer entities.
/// For a thermal resistance type, two temperatures are supplied, followed 
/// by some thermal conductance.
///
/// A heat transfer rate is added to each heat transfer entity's vector 
/// and then at each timestep advance, new temperatures are calculated 
///
///
/// A similar process is done for advection 
///
/// However, when parallel tubes are concerned, the heat transfer entity 
/// at the parallel tube end must split the heat evenly amongst n number 
/// of tubes. And if the heat is advected from n number of tubes, then 
/// we must multiply it back.
///
...
///
/// In this case, I bothered modelling the DHX tube side as parallel 
/// tubes rather than as a single tube 
///
/// doing so should decrease mass flowrate through each tube (slower)
/// thus making viscous forces more noticeable
/// 
///
#[test]
pub fn isolated_parallel_tubes_case_a_tchx_out_319_kelvin_46_celsius(){
    ...
    non_insulated_parallel_fluid_components::NonInsulatedParallelFluidComponent;
    ...
    fn verify_isolated_dhx_analytical_solution(
        ...
        Result<(),ThermalHydraulicsLibError>{
            ...
            fn dracs_fluid_mechanics_calc_mass_rate(
                ...
                dhx_tube_side_heat_exchanger_30: &NonInsulatedParallelFluidComponent,
            ...
            fn calculate_dracs_thermal_hydraulics(
                ...
                dhx_tube_side_heat_exchanger_30: &mut NonInsulatedParallelFluidComponent,
    ...
    verify_isolated_dhx_analytical_solution(
        ...
        0.00, // max error tolerance fraction
        ).unwrap();
    verify_isolated_dhx_analytical_solution(
