...
pub fn du_test_shell_and_tube_heat_exchanger_set_b(){
    ...
    use crate::tuas_boussinesq_solver::fluid_mechanics_correlations::
    ...
    use crate::tuas_boussinesq_solver::heat_transfer_correlations::nusselt_number_correlations::pipe_correlations::custom_gnielinski_turbulent_nusselt_correlation;
    ...
    // from data, is set B1
    // salt flow rate is 14.63 m3/h 
    // oil flow rate is 15.635 m3/h
    // T in salt is 214.93
    // T in oil is 74.49

    let vol_flowrate_salt = 
        VolumeRate::new::<cubic_meter_per_hour>(14.63);
    ...
    let (reynolds_num_b1,
        bulk_prandtl_b1,
        wall_prandtl_b1,
        nusselt_number_b1) = test_thread(clone_for_test_one,
        tube_inlet_temperature,
        shell_inlet_temperature,
        m_t,
        m_s);
        ...
        let test_two = thread::spawn(
            move || {
                ...
                let (reynolds_num_b2,
                    bulk_prandtl_b2,
                    wall_prandtl_b2,
                    nusselt_number_b2) = test_thread(clone_for_test_two,
                    tube_inlet_temperature,
                    shell_inlet_temperature,
                    m_t,
                    m_s);
