...
pub fn factory_test(){
    ...
    use crate::fluid_mechanics_lib::therminol_component::CalcPressureChange;
...
pub fn test_therminol_fldk_custom_component(){
    ...
    // now let's use the calc pressure change object
    use fluid_mechanics_lib::
    therminol_component::CalcPressureChange;
...
pub fn test_therminol_pipe(){
    let start = SystemTime::now();
    use fluid_mechanics_lib::therminol_component::therminol_pipe::
        DowthermAPipe;
...
pub fn test_dimensionless_number(){
    let bejan_d = 
        fluid_mechanics_lib::get_bejan_d(
    ...
    let reynolds_number = 
        fluid_mechanics_lib::get_reynolds_number(
    ...
    let custom_fldk = 
        fluid_mechanics_lib::CustomComponent::fldk(
            &custom_f,
            -5000.0,
            0.00014,
            10.0,
    ...
    let actual_bejan_custom_k = fluid_mechanics_lib::CustomComponent::
    ...
    let reynolds_number = fluid_mechanics_lib::CalcReynolds::from_mass_rate(
        fluid_massflowrate,
        pipe_xs_area,
        pipe_diameter,
        fluid_viscosity);
    ...
    let test_fluid_mass_flowrate = fluid_mechanics_lib::CalcReynolds::to_mass_rate(
        pipe_xs_area,
        reynolds_number,
        pipe_diameter,
        fluid_viscosity);
    ...
    let bejan_number = fluid_mechanics_lib::CalcBejan::from_pressure(
        fluid_pressure,
        pipe_diameter,
        fluid_density,
        fluid_viscosity);
    ...
    let test_fluid_pressure = fluid_mechanics_lib::CalcBejan::to_pressure(
        bejan_number,
        pipe_diameter,
        fluid_density,
        fluid_viscosity);
